...
$(function () {
    ...
    var connId;
    var switchControllId;
    for (i = 0; i < cantileverSuspendedId.length; i++)
    {
        connId = '#switch' + '-' + cantileverSuspendedId[i]
        switchControllId = '#' + cantileverSuspendedId[i]
        $(connId).bootstrapSwitch({
            onText: "ON",      // 设置ON文本
            offText: "OFF",    // 设置OFF文本
            onColor: "success",// 设置ON文本颜色     (info/success/warning/danger/primary)
            offColor: "info",  // 设置OFF文本颜色        (info/success/warning/danger/primary)
            ...
            //onSwitchChange: function (event, state) {
            //    if (state == true) {
            //        $(switchControllId).show();
            //    } else {
            //        $(switchControllId).hide();
            //    }
            //}
        });
    }
